...
//    压缩图片的最大高
     ...
     * @return              生成的图片文件
     */
    public static File makeImage(File imageFile, String url, String formatName, boolean compress){
        try {
            File output = new File(url);
            BufferedImage bufferedImage = ImageIO.read(imageFile);
//            如果不压缩，直接写入文件并返回
            if(!compress){
                ImageIO.write(bufferedImage, formatName, new FileOutputStream(output));
                return output;
            }
            Image newImage = getImage(bufferedImage);
//            获取压缩后图片的高与宽
...
//    返回一个Image对象，当参数bufferedImage的宽比高大的时候，使用最大的宽来压缩图片；
//    当参数bufferedImage的高比宽大的时候，使用最大的高来压缩图片
    private static Image getImage(BufferedImage bufferedImage){
        if(bufferedImage.getWidth() > bufferedImage.getHeight()){
            return bufferedImage.getScaledInstance(MAX_WIDTH, -10, Image.SCALE_AREA_AVERAGING);
        } else{
            return bufferedImage.getScaledInstance(-10, MAX_HEIGHT, Image.SCALE_AREA_AVERAGING);
        }
    }
